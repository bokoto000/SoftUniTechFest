...
// sensors 0 through 5 are connected to analog pins 0 through 5, respectively
...
void checkGround()
{
  //uses servos to put the sensor in the soil
  //checks moisture percentage
}
void sendData()
{
...
void PID()
{
  ...
  Serial.println(error); // prints current error to serial monitor ( used for testing)
...
void manual_calibration() {
  int i;
  for (i = 0; i < 250; i++)  // the calibration will take 250 * 0,02 = 5 seconds seconds
  {
    qtrrc.calibrate(QTR_EMITTERS_ON);
    delay(20); //delays program for 0,02 seconds
  }
}
void speedWrite(int right , int left)
{
